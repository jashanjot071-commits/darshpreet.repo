...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const savedPage    = localStorage.getItem('dk_current_page') || 'home';
  const savedProduct = localStorage.getItem('dk_current_product');
  const validPages   = ['home','shop','cart','wishlist','auth','about','contact','booking','checkout'];

  if(savedPage === 'product' && savedProduct) {
    openProduct(parseInt(savedProduct));          // re-opens detail page
  } else if(validPages.includes(savedPage)) {
    showPage(savedPage, true);                    // true = silent (no re-save)
  }

  // ── Handle browser back/forward (safe - won't work in iframes) ──
  try {
    window.addEventListener('popstate', () => {
      const hash = location.hash.replace('#','') || 'home';
      if(validPages.includes(hash)) showPage(hash, true);
    });
  } catch(e) {}
...
function showPage(pageId, silent = false) {
  ...
  // ── Persist state so refresh lands on same page ──
  if(!silent) {
    localStorage.setItem('dk_current_page', pageId);
    // Clear saved product if leaving product page
    if(pageId !== 'product') localStorage.removeItem('dk_current_product');
    // Update URL hash for browser back/forward support
    try { history.pushState(null, '', '#' + pageId); } catch(e) {}
  }
}
...
function openProduct(id) {
  ...
  if(!p) return;
  currentProduct = p;
  // Save so refresh restores this product
  localStorage.setItem('dk_current_page', 'product');
  localStorage.setItem('dk_current_product', id);
  try { history.pushState(null, '', '#product-' + id); } catch(e) {}
  const isWished = wishlist.some(w => w.id === p.id);
  const discount = p.originalPrice ? Math.round((1 - p.price/p.originalPrice)*100) : 0;
...
function renderCalendar() {
  ...
  const viewMonth = new Date(calYear, calMonth, 1);
  ...
  for(let d = 1; d <= daysInMonth; d++) {
    ...
    const isActive = selectedDay === d && !isPast;

    let cls = 'cal-day';
    if(isPast)   cls += ' inactive';
    if(isActive) cls += ' active';
    if(isToday && !isPast && !isActive) cls += ' today';
...
function changeMonth(delta) {
  ...
  const proposed = new Date(calYear, calMonth + delta, 1);
  const thisMonth = new Date(today.getFullYear(), today.getMonth(), 1);
...
function confirmBooking() {
  ...
  const dateStr = selectedDay + ' ' + monthNames[calMonth] + ' ' + calYear;
  ...
  const existing = JSON.parse(localStorage.getItem('dk_bookings') || '[]');
  existing.unshift(newBooking);
  localStorage.setItem('dk_bookings', JSON.stringify(existing));

  showToast('Booked! ' + svc.name + ' on ' + dateStr + ' at ' + selectedTime + ' 🎀', 'success');
